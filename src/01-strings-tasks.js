...
function encodeToRot13(str) {
  ...
  let result = '';
  const toEncode = str.split('');
  toEncode.forEach((element) => {
    switch (true) {
      case alph.indexOf(element) !== -1:
        result += alph[(alph.indexOf(element) + 13) % 26];
        break;
      case alphUp.indexOf(element) !== -1:
        result += alphUp[(alphUp.indexOf(element) + 13) % 26];
        break;
      default:
        result += element;
        break;
...
function getCardId(value) {
  ...
  for (let i = 0; i < deck.length; i += 1) {
    ...
    const index = deck.indexOf(element);
    if (element === value) {
      return index;
    }
  }
